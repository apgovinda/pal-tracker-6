pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE_NAME = "apgovinda/pal-tracker"
    }
    stages {
        stage('Build') {
            steps {
        ...
        stage('DeployToPKS') {
            ...
            steps {
                ...
                kubernetesDeploy(
                  ...
                  configs: 'pal-tracker-deployment.yml',
        ...
        stage('Get Service IP') {
            ...
            steps {
                //milestone(1)
                retry(10) {
                    withCredentials([usernamePassword(credentialsId: 'jx-pks', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                        script {
